package task1_hotel_booking.states;

import task1_hotel_booking.context.BookingContext;

public class IdleState implements BookingState {

    @Override
    public void selectRoom(BookingContext context, String roomNumber) {
        context.setSelectedRoom(roomNumber);
        context.setState(new RoomSelectedState());
        System.out.println("Номер " + roomNumber + " выбран.");
    }

    @Override
    public void changeRoom(BookingContext context, String newRoomNumber) {
        System.out.println("Ошибка: Сначала необходимо выбрать номер.");
    }

    @Override
    public void confirmBooking(BookingContext context) {
        System.out.println("Ошибка: Сначала необходимо выбрать номер.");
    }

    @Override
    public void pay(BookingContext context, double amount, double discount) {
        System.out.println("Ошибка: Сначала необходимо выбрать номер и подтвердить бронирование.");
    }

    @Override
    public void cancel(BookingContext context) {
        System.out.println("Нет активного бронирования для отмены.");
    }

    @Override
    public String getStateName() {
        return "Idle";
    }
}
